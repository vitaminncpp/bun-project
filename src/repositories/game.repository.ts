...
export async function updateOne(gameId: string, game: Partial<GameModel>): Promise<GameModel> {
  try {
    ...
    if (err instanceof Exception) throw err;
    throw new Exception(ErrorCode.GAME_UPDATE_FAILED, err?.message || "Error Updating Game", err);
  }
}
...
export async function findById(gameId: string): Promise<GameModel> {
  try {
    ...
    if (!game) {
      throw new Exception(ErrorCode.INVALID_GAME_ID, "Game does not Exists", gameId);
    }

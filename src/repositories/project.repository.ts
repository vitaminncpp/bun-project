...
export async function insertOne(project: ProjectModel): Promise<ProjectModel> {
  try {
    ...
    if (!inserted || inserted.length === 0) {
      throw new Exception(ErrorCode.RECORD_INSERTION_FAILED, "Error Creating Project", project);
    }
...
export async function findById(projectId: string): Promise<ProjectModel> {
  try {
    ...
    if (!project) {
      throw new Exception(ErrorCode.PROJECT_NOT_EXIST, "Project does not Exists", projectId);
    }
...
export async function findAll(options: { page: number; size: number }): Promise<{
  total: number;
  page: number;
  size: number;
  ...
  try {
    const projects = await db
      ...
      .limit(options.size);
    if (!projects) {
      throw new Exception(ErrorCode.PROJECT_NOT_EXIST, "Error Getting all Projects");
    }
    const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(projectTable);
...
export async function removeOne(projectId: string): Promise<ProjectModel> {
  const project = await db.delete(projectTable).where(eq(projectTable.id, projectId)).returning();
